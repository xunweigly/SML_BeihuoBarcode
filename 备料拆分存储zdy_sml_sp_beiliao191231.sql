USE [UFDATA_888_2014]
GO
/****** Object:  StoredProcedure [dbo].[zdy_sml_sp_beiliao]    Script Date: 12/31/2019 08:25:56 ******/
SET ANSI_NULLS ON
GO
...
DECLARE @iqty1  DECIMAL(18,0),@iqty2  DECIMAL(18,0),@iqty3  DECIMAL(18,0),@iqty4  DECIMAL(18,0),@iqty5  DECIMAL(18,2),@iqty6  DECIMAL(18,0)
...
WHERE a.iappids = b.AutoID AND a.id = @id  AND cinvcode = @cinvcode AND b.ID=c.ID
		...
		WHILE @imin2<=@imax2
			...
			SELECT @cmocode1  =''
				...
				SELECT @cmocode1  = cmocode+'-'+imoseq+'('+CONVERT(VARCHAR(20), ceiling(iquantity))+')',@iqty1  =iquantity  FROM #tmp2  WHERE id2 = @imin2
				SELECT @cmocode2  = cmocode+'-'+imoseq+'('+CONVERT(VARCHAR(20), ceiling(iquantity))+')',@iqty2  =iquantity FROM #tmp2  WHERE id2 = @imin2+1
				SELECT @cmocode3  = cmocode+'-'+imoseq+'('+CONVERT(VARCHAR(20), ceiling(iquantity))+')',@iqty3  =iquantity FROM #tmp2  WHERE id2 = @imin2+2
				SELECT @cmocode4  = cmocode+'-'+imoseq+'('+CONVERT(VARCHAR(20), ceiling(iquantity))+')',@iqty4  =iquantity FROM #tmp2  WHERE id2 = @imin2+3
				SELECT @cmocode5 = cmocode+'-'+imoseq+'('+CONVERT(VARCHAR(20), ceiling(iquantity))+')',@iqty5  =iquantity FROM #tmp2  WHERE id2 = @imin2+4
				SELECT @cmocode6 = cmocode+'-'+imoseq+'('+CONVERT(VARCHAR(20), ceiling(iquantity))+')',@iqty6  =iquantity FROM #tmp2  WHERE id2 = @imin2+5
